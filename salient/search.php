				 ...
				 <div class="row">
					<div class="col span_6 ">
						<div class="inner-wrap">
							<h1 style="color: <?php echo esc_attr( $search_results_header_font_color ); ?>;"><?php echo esc_html__( 'Результат запроса', 'salient' ); ?> <span>"<?php echo esc_html( get_search_query( false ) ); ?>"</span></h1>
							<?php
							if ( $wp_query->found_posts ) {
								echo '<span class="result-num" style="color: ' . esc_attr( $search_results_header_font_color ) . ';">' . esc_html__( 'Найдено результатов: ', 'salient' ) . ' ' . esc_html( $wp_query->found_posts ) . '</span>'; }
							?>
			...
			<div class="col span_<?php echo esc_attr( $search_col_span ); // WPCS: XSS ok. ?>">
				...
				<div id="search-results" data-layout="<?php echo esc_attr( $search_results_layout ); ?>">
							...
							if ( get_post_type( $post->ID ) == 'post' ) {
								...
								<article class="result" data-post-thumb="<?php echo esc_attr( $using_post_thumb ); ?>">
									<div class="inner-wrap">
										...
										?>
										<h2 class="title"><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a> <span><?php echo esc_html__( 'Страница', 'salient' ); ?></span></h2>
										<?php
								...
								<article class="result" data-post-thumb="<?php echo esc_attr( $using_post_thumb ); ?>">
									<div class="inner-wrap">
										...
										?>
										<h2 class="title"><a href="<?php echo esc_url( $nectar_portfolio_project_url ); ?>"><?php the_title(); ?></a> <span><?php echo esc_html__( 'Портфолио', 'salient' ); ?></span></h2>
									</div>
								</article><!--/search-result-->
								<?php
								...
								<article class="result" data-post-thumb="<?php echo esc_attr( $using_post_thumb ); ?>">
									<div class="inner-wrap">
										...
										?>
										<h2 class="title"><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a> <span><?php echo esc_html__( 'Товар', 'salient' ); ?></span></h2>
										<?php
										if ( $search_results_layout == 'list-no-sidebar' ) {
											the_excerpt(); }
										?>
					...
					endwhile;
						...
						echo '<h3>' . esc_html__( 'По данному запросу ничего не найдено', 'salient' ) . '</h3>';
						echo '<p>' . esc_html__( 'Измените запрос и повторите поиск', 'salient' ) . '</p>';
						get_search_form();
				...
				<?php if ( get_next_posts_link() || get_previous_posts_link() ) { ?>
					<div id="pagination" data-layout="<?php echo esc_attr( $search_results_layout ); ?>">
						<div class="prev"><?php previous_posts_link( '&laquo; Предыдущий запрос' ); ?></div>
						<div class="next"><?php next_posts_link( 'Следующий запрос &raquo;', '' ); ?></div>
					</div>
				<?php } ?>
