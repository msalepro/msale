...
if( !function_exists('be_adjacent_post_link') ) {
	...
	function be_adjacent_post_link($format, $link, $in_same_cat = false, $excluded_categories = '', $previous = true, $taxonomy = 'category') {
		...
		if ( empty($post->post_title) )
			$title = $previous ? esc_html__('Предыдущая страница','salient') : esc_html__('Следующая страница ','salient');
		...
		$adjacent = $previous ? 'Предыдущий' : 'Следующий';
		...
		if( !empty($nectar_options['extra_pagination']) && $nectar_options['extra_pagination'] == '1' ){
			    ...
			    if ($total_pages > 1){
			      ...
			      echo paginate_links(array(
			          ...
			          'current' => $current,
			          'total' => $total_pages,
			          'prev_text'    => esc_html__('Предыдущий','salient'),
    				  	'next_text'    => esc_html__('Следующий','salient')
			        ));
